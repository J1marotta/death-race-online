...
- The background music plays continuously from the landing page through the lobby, countdown, and into live play, so no screen is silent and the music does not stop and restart between screens: soft sine-pad seventh chords, a gentle triangle melody, and a quiet bass note per chord change. Because browsers block audio before a gesture, the landing music begins on the first click or keypress. The music, footsteps, and cue levels are balanced to stay clearly audible under the louder gunshot rather than sitting near silence, and the audio context is resumed whenever music starts so play is never silent when the phase changes without a fresh gesture.
- The audio controls — a mute toggle and a volume slider that drives a single master output gain — appear on every screen that has sound: the landing page header and the room top bar (lobby, live play, and results). Changing the volume adjusts everything live and the slider is disabled while muted.
